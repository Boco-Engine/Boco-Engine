...
DEBUG_MESSENGER_CALLBACK :: proc(
    ...
    logger := cast(^log.Logger)pUserData
	...
	logger.procedure(logger.data, level, string(pCallbackData.pMessage), logger.options)
...
fill_debug_messenger_info :: proc(debug_messenger_info: ^vk.DebugUtilsMessengerCreateInfoEXT) {
    ...
    debug_messenger_info.pUserData = cast(rawptr)context.user_ptr
}
...
RendererInternals :: struct {
    ...
    uniform_buffer_objects: []UniformBufferObject,
    uniform_buffers: []BufferResources,
    camera_buffers: []BufferResources,
...
init_device :: proc(using renderer: ^Renderer, layers, extensions: []cstring) -> (ok: bool = false) {
    ...
    for feature in SupportedRendererFeatures {
        if feature not_in enabled_features {
            continue
        }
        switch feature {
            case .geometryShader:
...
init_render_pass :: proc(using renderer: ^Renderer) -> bool {
    ...
    attachment_descriptions[1].format = .D32_SFLOAT
